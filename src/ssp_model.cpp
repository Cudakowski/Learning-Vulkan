...
{
template<>
struct hash<ssp::SspModel::Vertex> {
    size_t operator()(ssp::SspModel::Vertex const &vertex) const{
        size_t seed = 0;
        ssp::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
        return seed;
    }
};
} // namespace std
...
void  SspModel::Builder::loadModel(const std::string &filepath)
{
